...
ドライブ段は常にONのままなので，受信時にもバイアス電流が流れています.  
LTspiceを使用して見積もったところ，12V電源でコレクタ電流が27mA (320mW), 5V電源で7.7mA(39mW)でした.  
受信時にこれではもったいないので，ドライブ段もON/OFFすることにしました.  
また, CWモードでは, 受信時もSi5351がONのままで局発信号が漏れていますので, その制御も行うことにしました.  

#### 方法0：CWモードのみ改良, デジタルモードは従来通り
	...
	しかしこの方法では, デジタルモードでのドライブ段のON/OFFはできません.  
	このため, 電鍵関係の配線を少し変える必要があります(方法1あるいは方法2).  
	配線が前のままでも, 新しいスケッチ”QP-7C_FT8a.ino”は, CWモードで作動します(キーがオンの時だけSi5351の出力がON).
	デジタルモードでも使用できますが, この配線のままでは受信時にもバイアス電流が流れっぱなしです.  

#### 方法1：Arduino Nano互換機のデジタル出力でKEY端子を直接制御　　
	電鍵端子はD2ピンに接続し、電鍵の状態をArduino Nano互換機で検出します(前と同じ).   
	送信のON/OFFをArduino Nano互換機のデジタル出力(D12ピン)で制御します.  
	...
	ドライブ段のベースバイアス電圧は12V時でも3.8Vなので，受信時はエミッターへの5V印加でトランジスタがOFFになります.  

#### 方法2：Arduino Nano互換機のデジタル出力でKEY端子をオープンコレクタ接続を使用して制御（標準的かつ着実な方法）　　
	電鍵端子はD2ピンに接続し、電鍵の状態をArduino Nano互換機で検出します.   
	送信のON/OFFをArduino Nano互換機のデジタル出力(D13ピン)で制御します.  
...
#### Arduino Nano用スケッチ例は，”QP-7C_FT8a.ino”です．  
	D2ピンを電鍵入力に使用しました．  
	D12ピンを送受信の切り替え（送信時LOW）に使用しました(方法1)．  
	D13ピンを送受信の切り替え（送信時HIGH）に使用しました(方法2)．  
	方法1か方法2で接続します(実は方法1しか試していません.).
	CW運用時も電鍵で直接”KEY”端子をON/OFFするのではなく, Arduino Nano互換機を介して切り替えます(方法0以外).  
	CW運用の受信時にSi5351のCLK0出力をOFFにしましたので,トランシーバーとして使用しやすくなりました.  
	CWモードで,電鍵をジャックから抜くと送信します.  
