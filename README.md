...
### Windows版WSJT-Xでの設定法  
	...
	PTT方式は，”VOX”を選択  	
	モードは，”Data/Pkt”を選択すると周波数を変更した際にFSKモードに移行します．”指定なし”を選択すると周波数を変更してもモードは変わりません．  	
	ただし，USBシリアルを繋いだ後でWSJT-Xを立ち上げた場合とポーリングが行われた場合は， この選択に関係なく強制的にFSKモードに移行させられるようです．  
コンパイル済みのWSJT-Xでは，現時点で，ポーリングが行われないようには設定できません．  

### EEPROMを使用して，エレキーの速度設定および周波数を記憶するようにしました．  
エレキーの速度は，設定変更した際にEEPROMに書き込まれます．  
周波数については，それぞれのモードで送信が行われた際にEEPROMに書き込むようにしました．  
