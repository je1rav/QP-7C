...
## <del>QP-7Cミニ送信機の改造（その4）の追加 その5 （WSJT-X ver2.5.4のCAT対応　不完全な対応でした）</del>
対応しました．　   
### QP-7Cミニ送信機の改造（その6）の追加 （WSJT-XのCAT対応とEEPROMの使用）
を参照ください．   
...
## <del>QP-7Cミニ送信機の改造（その5）の追加 （WSJT-X ver2.5.4のCAT対応　不完全な対応でした）</del>
対応しました．　   
### QP-7Cミニ送信機の改造（その6）の追加 （WSJT-XのCAT対応とEEPROMの使用）
を参照ください．  
...
## QP-7Cミニ送信機の改造（その6）の追加 （WSJT-XのCAT対応とEEPROMの使用）
スケッチ"QP-7C_TRX-PCB.ino"および"QP-7C_TRX2.ino”をアップデートしました．  

WSJT-XのCATにほぼ対応しました．  
Windows版では作動しますが，残念ながらOSX版ではシリアルテストを行う際にリセットがかかってエラーが出てうまく作動しません．  
EEPROMを使用して，エレキーの速度設定および送信周波数を記憶するようにしました．  

## QP-7Cミニ送信機の改造（その6）の追加2 （バグフィックス）
スケッチ"QP-7C_TRX-PCB.ino"および"QP-7C_TRX2.ino”をアップデートしました．  

前回の「QP-7Cミニ送信機の改造（その6）の追加」のファームウエアに重大なバグがあり，このバグを修正しました．  
「QP-7Cミニ送信機の改造（その5）」のCD2003使用のスーパーヘテロダイン受信機（"QP-7C_TRX2.ino”使用）では，受信できない状態でした．  
「QP-7Cミニ送信機の改造（その4）」のSA612使用のダイレクトコンバージョン受信機（"QP-7C_TRX-PCB.ino"使用）では，受信は可能でしたがCATでのmode切り替えにバグがありました．  

### Windows版WSJT-XでのCATの設定法  
	ファイル＞設定で設定ウインドウが開きます  
	無線機(R)タグで，  
	無線機：”Kenwood TS-2000”を選択  
	ポーリング間隔：基本的に周波数変更しか使いませんので長くしておいてください(ex.90s, 送信中にはなるべくポーリングされたくない)． 
	シリアルポートはArduino nano互換機の対応するシリアルポートを選択  
	ボーレート：”115200”を選択  
	それ以外のシリアルポートパラメーターはデフォルトのまま  
	PTT方式は，”VOX”を選択  	
	モードは，”USB”を選択すると周波数を変更した際にFSK（デジタル）モードに移行します．”指定なし”を選択すると周波数を変更してもモードは変わりません．
	（”Data/Pkt”ではなく，”USB”を選択してください（前回から変わっています．）．）．
	コンパイル済みのWSJT-Xでは，現在，ポーリングを行わないようには設定できません．

### EEPROMの使用方法  
	エレキーの速度は，設定変更した際にEEPROMに書き込まれます．  
	周波数については，それぞれのモードで送信が行われた際にEEPROMに書き込むようになっています．  
	このため，CWモードおよびFSKモードのそれぞれについて，最後に送信された周波数が立ち上げ時に設定されます．  
	このEEPROM機能を使わない場合は，プログラムの8行目にある  
	#define IuseEEPROM  
	文を，  
	//#define IuseEEPROM  
	のようにコメント文に変更して，コンパイルして下さい．  
	EEPROMを使用しないで，エレキーの速度の初期値を変えたい場合は，  
	int cw_rate= 20; //WPM  
	文の値(現在値20)を変更して，コンパイルして下さい．  

## QP-7Cミニ送信機の改造（その6）の追加3 （バグフィックス）
スケッチ"QP-7C_TRX-PCB.ino"および"QP-7C_TRX2.ino”をアップデートしました．  

EEPROMの書き込みデータに異常が発生した場合，次の立ち上げ時にプリセット値に戻るように修正しました(Tnx. JG2CEZ)．  


## QP-7Cミニ送信機の改造（その6）の追加4 （バグフィックス）
スケッチ"QP-7C_TRX-PCB.ino"および"QP-7C_TRX2.ino”をアップデートしました．  

ロータリースイッチによる周波数変更時において，変更する桁を切り替えるためのプッシュスイッチについて，長押しか短押しかの判定ルーチンを改善しました(Tnx. JG2CEZ)．  
